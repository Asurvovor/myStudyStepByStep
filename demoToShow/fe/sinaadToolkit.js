...
(function (window, undefined) {
    ...
    sinaadToolkit.RESOURCE_URL = sinaadToolkit.RESOURCE_URL || [
        ...
        '//d2.sina.com.cn/litong/zhitou/sinaads',
        '//d3.sina.com.cn/litong/zhitou/sinaads',
        '//d4.sina.com.cn/litong/zhitou/sinaads',
        '//d5.sina.com.cn/litong/zhitou/sinaads',
        '//d6.sina.com.cn/litong/zhitou/sinaads',
        '//d7.sina.com.cn/litong/zhitou/sinaads',
        '//d8.sina.com.cn/litong/zhitou/sinaads',
        '//d9.sina.com.cn/litong/zhitou/sinaads'
        //'.'
    ...
    sinaadToolkit.url = sinaadToolkit.url || /** @lends sinaadToolkit.url */{
         ...
         * 判断是否有http或者https开头，如果没有直接认定添加http或者https(现改为如果没有直接添加//)
         * @todo \n\r 去除
         */
        ensureURL : function (source) {
            source = sinaadToolkit.string.trim(source);
            return source ? (/^((http|https):)*\/\//).test(source) ? source : ('//' + source) : '';
        },
        /**
         * 创建一个url
        ...
        fortp: function (el, e) {
            ...
            if (link.indexOf('//sax.sina.com.cn') !== -1 || link.indexOf('//saxn.sina.com.cn') !== -1) {
                el.href = link + '&am=' + encodeURIComponent('{clkx:' + pageX + ',clky:' + pageY + '}');
            }
            return true;
        }
    };
    ...
    sinaadToolkit.sio = sinaadToolkit.sio || (function () {
        ...
        return /** @lends sinaadToolkit.sio */{
            ...
            jsonp : function (url, callback, optOptions) {
                ...
                if (sinaadToolkit.isString(callback)) {
    ...
    sinaadToolkit.swf = sinaadToolkit.swf || /** @lends sinaadToolkit.swf */{
        ...
        createHTML : function (options) {
            ...
            //如果尺寸小于398*298，在chrome的45以上版本会自动暂停播放flash
        ...
        hackAutoPlay: function (element, html) {
            //如果匹配中<!--fakesize:id|width|height-->开头的
            // var fake, flash, positionCache, insertCache, widthCache;
            // if ((fake = html.match(/<\!--\s*fakesize\:([0-9a-zA-Z_]+)\|(\d+)\|(\d+)\s*-->/))) {
            //     positionCache = element.style.position;
            //     widthCache = element.style.width;
            //     element.style.position = 'absolute';
            //     element.style.opacity = 0;
            //     element.style.width = 'auto';
            //     element.innerHTML = html;

            //     insertCache = document.createElement('div');
            //     element.parentNode.insertBefore(insertCache, element);
            //     //挂到body里面，防止因为外部容器设置了宽度导致flash依然因为尺寸不够无法自动播放
            //     document.body.appendChild(element);

            //     setTimeout(function () {
            //         flash = sinaadToolkit.swf.getMovie(fake[1]);
            //         if (flash) {
            //             flash.width = fake[2];
            //             flash.height = fake[3];
            //         }
            //         element.style.position = positionCache || 'relative';
            //         element.style.width = widthCache || 'auto';
            //         element.style.opacity = 1;

            //         insertCache.parentNode.insertBefore(element, insertCache);
            //         insertCache.parentNode.removeChild(insertCache);
            //         insertCache = null;
            //     }, 500);
            // } else {
                element.innerHTML = html;
                ...
                if (doc) {
                    ...
                    try {
                        ...
                        content = sinaadToolkit.browser.ie && sinaadToolkit.browser.ie <= 6 ?
                            ...
                            "/*document.close();*/";
                        var tagname = 'script';
                        iframe.src = 'javascript:\'<script type="text/javascript">' + content + "\x3c/" + tagname + ">'";
                    } catch (e) {
                        window[key] = null;
            ...
            for (; i < len; i++) {
                ...
                switch (type) {
                    case 'image' :
                        ...
                        _html = link ? '<a style="display:block;line-height:0;" href="' + link + '" target="_blank" data-link="' + link + '" onmousedown="return sinaadToolkit.url.fortp(this, event);">' + _html + '</a>' : _html;
                        break;
                    case 'text' :
                        _html = link ? '<a href="' + link + '" target="_blank" data-link="' + link + '" onmousedown="return sinaadToolkit.url.fortp(this, event);">' + src + '</a>' : src;
                        //_html = link ? '<i style="font-style:normal;">广告:</i><a href="' + link + '" target="_blank" data-link="' + link + '" onmousedown="return sinaadToolkit.url.fortp(this, event);">' + src + '</a>' : '<i style="font-style:normal;">广告:</i>' + src;
                        break;
                    case 'flv' :
                    case 'video' :
                        ...
                        if (link) {
                            _html = [
                                //'<div style="width:' + width + ';height:' + height + ';position:relative;overflow:hidden;">',
                                    _html,
                                    '<a style="position:absolute;background:#fff;opacity:0;filter:alpha(opacity=0);width:' + width + ';height:' + height + ';left:0;top:0" href="' + link + '"  data-link="' + link + '" target="_blank"  onmousedown="return sinaadToolkit.url.fortp(this, event);"></a>'
                                //'</div>'
                            ].join('');
                        }
                        break;
                    ...
                    case 'url' :
                        ...
                        break;
                    case 'js' :
                    case 'extend' ://扩展类:extend,用于针对扩展画中画,扩展通栏等扩展embed广告,解决为embed扩展广告添加的时序在更改为普通embed广告时造成的时序不能被置为done的问题
                        _html = [
