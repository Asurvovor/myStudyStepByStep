(function (window, sinaadToolkit, mediaControl, undefined) {
    "use strict";
    //常量定义
    var SIDE_CLOSE_BTN = '//d2.sina.com.cn/litong/zhitou/sinaads/60x18_1_close.gif',
        MAIN_CLOSE_BTN = "//d2.sina.com.cn/litong/zhitou/sinaads/75x22-close.gif",
        SIDE_CLOSE_BTN_SIZE = [120, 18],
        MAIN_CLOSE_BTN_SIZE = [75, 22],
        MAIN_SIZE = [1000, 90],
        SIDE_SIZE = [120, 270];
    /**
     * 跨栏广告
    ...
    function CoupletMedia(config) {
        ...
        sinaadToolkit.ad.embed(
            rightContent,
            config.type[2] || config.type[1] || config.type[0],
            config.sideWidth,
            config.sideHeight,
            sinaadToolkit.ad.createHTML(
                config.type[2] || config.type[1] || config.type[0],
                config.src[2] || config.src[1] || config.src[0],
                config.sideWidth,
                config.sideHeight,
